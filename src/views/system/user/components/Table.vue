      ...
      <el-table-column fixed="right" label="操作" width="160">
        <template #default="scope">
          <el-button
            ...
            :icon="EditPen"
            circle
            @click="handleInvokeOption(scope.row.id, scope.row.username)"
          />
...
const handleInvokeOption = (id: any, username: string) => {
  table.invoke.open = true
  table.invoke.id = id
  table.invoke.username = username
}
const handleInvokeOpen = () => {
  refInvoke.value.init(table.invoke.id, table.invoke.username)
}
